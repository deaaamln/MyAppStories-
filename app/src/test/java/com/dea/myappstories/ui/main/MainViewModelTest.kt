...
import com.dea.myappstories.MainDispatcherRule
...
import com.dea.myappstories.utils.DataDummy
import com.dea.myappstories.utils.getOrAwaitValue
...
class MainViewModelTest {
    ...
    fun `when Get Stories Should Not Null and Return No Data` () = runTest {
        ...
        Mockito.`when`(storyRepository.getStories()).thenReturn(expectedStories)
        val mainViewModel = MainViewModel(storyRepository)
        val actualStories: PagingData<ListStoryItem> = mainViewModel.getListStories.getOrAwaitValue()
        val differ = AsyncPagingDataDiffer(
            diffCallback = ListStoryAdapter.DIFF_ITEM_CALLBACK,
            updateCallback = noopListUpdateCallback,
            workerDispatcher = Dispatchers.Main,
